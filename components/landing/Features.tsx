...
export default function Features() {
  ...
  return (
    <section id="features" className="relative bg-surface-alt py-section border-b border-outline">
      <div className="mx-auto max-w-[1160px] px-4 lg:px-margin">
        ...
        >
          ...
          <div className="caption text-on-surface-variant flex items-center gap-2 select-none mb-4">
            <span className="text-primary text-[10px]">●</span> Features
          </div>
          
          {/* Headline */}
          <h2 className="display-md text-on-surface mb-lg">
            Built to talk like your best sales agent
          </h2>
          
          {/* Subheadline (body-lg, paired with display-md) */}
          <p className="body-lg text-on-surface-variant max-w-[65ch]">
            An AI voice assistant that actually understands the flow of Indian business conversations and handles them from hello to closing.
          </p>
        </motion.div>

        {/* Bento Grid layout from features-8 */}
        <motion.div
          variants={containerVariants}
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true, margin: "-100px" }}
          className="relative z-10 grid grid-cols-6 gap-6"
        >
          {/* Card 1: Sub-2.5s Voice Response (Customizable visualizer) */}
          <motion.div
            variants={cardVariants}
            whileHover={{ y: -4 }}
            className="relative col-span-full flex flex-col justify-between overflow-hidden lg:col-span-2 bg-surface border border-outline rounded-xl p-8 shadow-[0_2px_8px_rgba(26,26,26,0.06)]"
          >
            <div className="relative m-auto size-fit py-6">
              <div className="relative flex h-24 w-56 items-center">
                <svg
                  className="text-outline/40 absolute inset-0 size-full"
                  viewBox="0 0 254 104"
                  fill="none"
                ...
                <span className="mx-auto block w-fit text-5xl font-semibold font-display text-on-surface">
                  &lt;2.5s
                </span>
              </div>
            </div>
            <div className="mt-8 space-y-2">
              ...
              <p className="body-md text-on-surface-variant">
                Zero awkward silence. Telephony pipelines ensure an ultra-fast response time under 2.5 seconds, making conversations feel natural and human.
              </p>
            </div>
          </motion.div>

          {/* Card 2: Lead Qualification (Concentric Target Visualizer) */}
          <motion.div
            variants={cardVariants}
            whileHover={{ y: -4 }}
            className="relative col-span-full overflow-hidden sm:col-span-3 lg:col-span-2 bg-surface border border-outline rounded-xl p-8 shadow-[0_2px_8px_rgba(26,26,26,0.06)] flex flex-col justify-between"
          >
            <div className="pt-6 flex justify-center">
              <div className="relative flex aspect-square size-32 rounded-full border border-outline before:absolute before:-inset-2 before:rounded-full before:border before:border-outline/50">
                <svg
                  ...
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <circle cx="50" cy="50" r="40" stroke="currentColor" strokeWidth="2" strokeDasharray="4 4" className="text-outline" />
                  <circle cx="50" cy="50" r="30" stroke="currentColor" strokeWidth="2" className="text-outline" />
                  <circle cx="50" cy="50" r="20" stroke="currentColor" strokeWidth="2" className="text-primary-dark" />
                  <circle cx="50" cy="50" r="10" fill="currentColor" className="text-primary" />
                  <line x1="50" y1="5" x2="50" y2="95" stroke="currentColor" strokeWidth="1" className="text-outline/40" />
                  <line x1="5" y1="50" x2="95" y2="50" stroke="currentColor" strokeWidth="1" className="text-outline/40" />
                </svg>
              </div>
            </div>
            <div className="mt-8 space-y-2 text-center">
              ...
              <p className="body-md text-on-surface-variant">
                Saral AI doesn't just answer; it qualifies callers, filtering high-intent leads, budget fits, and urgent requests automatically.
              </p>
            </div>
          </motion.div>

          {/* Card 3: 24/7 Availability (Continuous Area Graph Visualizer) */}
          <motion.div
            variants={cardVariants}
            whileHover={{ y: -4 }}
            className="relative col-span-full overflow-hidden sm:col-span-3 lg:col-span-2 bg-surface border border-outline rounded-xl p-8 shadow-[0_2px_8px_rgba(26,26,26,0.06)] flex flex-col justify-between"
          >
            ...
            <div className="mt-8 space-y-2 text-center">
              ...
              <p className="body-md text-on-surface-variant">
                Never miss another business call. Saral AI is active 24/7, answering immediately and handling unlimited simultaneous inquiries.
              </p>
            </div>
          </motion.div>

          {/* Card 4: Multilingual support (2-column layout with wave path visualizer) */}
          <motion.div
            variants={cardVariants}
            whileHover={{ y: -4 }}
            className="relative col-span-full lg:col-span-3 bg-surface border border-outline rounded-xl p-8 shadow-[0_2px_8px_rgba(26,26,26,0.06)] flex flex-col justify-between overflow-hidden"
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
              <div className="space-y-4">
                <h3 className="display-sm text-[22px] text-on-surface font-display">
                  Multilingual by Default
                </h3>
                <p className="body-md text-on-surface-variant">
                  Speak to your customers in their language. Saral AI fluidly switches between Hindi, Hinglish, and English to match their preference automatically.
                </p>
              </div>
              <div className="rounded-xl border border-outline bg-surface-alt p-4 relative overflow-hidden">
                <div className="absolute left-3 top-2 flex gap-1">
                  <span className="block size-2 rounded-full bg-outline"></span>
                  <span className="block size-2 rounded-full bg-outline"></span>
                  <span className="block size-2 rounded-full bg-outline"></span>
                </div>
                <div className="pt-4 flex flex-col gap-3 font-mono text-xs">
                  <div className="flex gap-2 items-start text-on-surface-variant">
                    <span className="text-secondary font-bold">Caller:</span>
                    <span>"Mujhe pricing details chahiye thi..."</span>
                  </div>
                  <div className="flex gap-2 items-start text-on-surface">
                    <span className="text-primary-dark font-bold">Saral AI:</span>
                    <span>"Haan bilkul, humare teen plan hain..."</span>
                  </div>
                </div>
                <svg
                  className="w-full h-16 mt-4 text-primary"
                  viewBox="0 0 366 231"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
          ...
          {/* Card 5: WhatsApp Summaries (2-column layout with status/name items) */}
          <motion.div
            variants={cardVariants}
            whileHover={{ y: -4 }}
            className="relative col-span-full lg:col-span-3 bg-surface border border-outline rounded-xl p-8 shadow-[0_2px_8px_rgba(26,26,26,0.06)] flex flex-col justify-between overflow-hidden"
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
              <div className="space-y-4">
                ...
                <p className="body-md text-on-surface-variant">
                  Skip listening to long call recordings. Get an instant, structured summary on WhatsApp with caller details, qualified intents, and clear next steps.
                </p>
              </div>
              <div className="relative mt-6 md:-my-8 md:-mr-8 md:ml-6 flex h-full flex-col justify-center space-y-4 py-6 md:border-l border-outline/50 pl-6">
                <div className="relative flex w-full items-center justify-end gap-2 pr-6">
                  <span className="block h-fit rounded-full border border-outline bg-surface-alt px-3 py-1 text-xs text-on-surface-variant shadow-sm">
                    Aarav S. (New Lead)
                  </span>
                  <div className="size-7 bg-primary rounded-full flex items-center justify-center text-[10px] font-bold text-on-primary">
                    AS
                  </div>
                </div>
                <div className="relative flex items-center gap-2">
                  <div className="size-8 bg-secondary rounded-full flex items-center justify-center text-xs font-bold text-on-secondary">
                    PP
                  </div>
                  <span className="block h-fit rounded-full border border-outline bg-surface-alt px-3 py-1 text-xs text-on-surface-variant shadow-sm">
                    Priya P. (Qualified)
                ...
                <div className="relative flex w-full items-center justify-end gap-2 pr-6">
                  <span className="block h-fit rounded-full border border-outline bg-surface-alt px-3 py-1 text-xs text-on-surface-variant shadow-sm">
                    Rajesh K. (Pricing Query)
                  </span>
                  <div className="size-7 bg-tertiary rounded-full flex items-center justify-center text-[10px] font-bold text-on-tertiary">
                    RK
                  </div>
                </div>
              </div>
